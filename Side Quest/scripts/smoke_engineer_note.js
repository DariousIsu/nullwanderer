...
ok(/strip\.id = 'zoe-strip'/.test(chatSrc) && /\['stimulation', 'stim'\], \['social', 'him'\], \['curiosity', 'curious'\], \['energy', 'energy'\], \['progress', 'progress'\]/.test(chatSrc) && /window\.sq\.onConsciousnessState\(\(s\) =>/.test(chatSrc) && /camera-onair/.test(chatSrc.split("strip.id = 'zoe-strip'")[1]) && /he last spoke \$\{fmt\(c\.since_his_word_min\)\} ago/.test(chatSrc), 'the strip is drawn beside the camera light and carries felt time and the wondering in its title');
