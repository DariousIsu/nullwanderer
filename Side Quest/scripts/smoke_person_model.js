...
ok(/source = 'personal_fact' ORDER BY created_ts DESC LIMIT 200/.test(mainS) && /PM\.seedOwner\(\{ knownFacts: facts \}\)/.test(mainS) && /db\.init\(\);\s+_seedPersonModel\(\);/.test(mainS) && /if \(!PM\.get\('owner'\)\) _seedPersonModel\(\);/.test(mainS), 'his row is seeded after the database opens (boot p318: a seed at module load found no database) and lazily on the first turn');
