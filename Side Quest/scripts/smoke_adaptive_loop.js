...
(async () => {
  ...
  // fires; the REFINEMENT branch must enter through judgment instead and arm paper mode.
  {
    ...
    // The Word deliverable reaches the CANVAS, not just the notes dir (Lucas 2026-08-07).
    ok(/extractDocxHtml\(_abs\)/.test(m) && m.indexOf("blockType: 'document_file'") > -1, 'the rendered paper .docx lands on the canvas as a document_file block');
