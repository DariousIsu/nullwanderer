...
class EchoSuit {
  ...
  suitContextBlock() {
    ...
    const lines = [
      ...
      `• <echo-do name="tool">{json}</echo-do> — run a specific tool by name yourself (only when you already know exactly which one). To find a person/org/bill BY NAME in the graph use search_entities (an FTS index — returns instantly, richer than a row); for contact rosters use list_contacts / search_contacts / contact_facets. NEVER look an entity up with a db_query name LIKE — the entities table is 1.76M rows and a name LIKE full-scans ALL of them (~12s, and it dies outright on the default timeout). Hand-written db_query SQL is the LAST resort: if you must, filter on an actually-indexed column (state / type / external_id / wikidata_qid), never a name LIKE, and pass "timeout_seconds":20.`,
      `• <echo-delegate name="agent">task</echo-delegate> — hand a heavy job to a background agent.`,
    ...
    lines.push(`A COUNT or LIST of OUR data ("how many LAMP members", "list the committee", "who is OURS") almost ALWAYS has a recipe — check your menu or <echo-find> it (it lists recipes first). If a lookup misses, do NOT give up or drift to another topic: escalate IN THIS ORDER — a broader recipe → <echo-find> in different words → a named read tool (search_entities to find something by name, list_contacts / search_contacts for contact rosters) → only then a hand-written db_query (an indexed filter on state/type/id, NEVER a name LIKE, "timeout_seconds":20) — or tell Lucas you couldn't find it. FINISH his question before anything else.`);
    return lines.join('\n');
  }
