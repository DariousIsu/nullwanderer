...
class EchoSuit {
  ...
  suitContextBlock() {
    ...
    const lines = [
      ...
      `• <echo-do name="tool">{json}</echo-do> — run a specific tool by name yourself (only when you already know exactly which one). For people/rosters use the NAMED read tools (list_contacts, search_contacts, contact_facets) — hand-written db_query SQL is the LAST resort: if you must, use ONE indexed filter (state/name/type), never a leading-wildcard LIKE over everything, and pass "timeout_seconds":20.`,
      `• <echo-delegate name="agent">task</echo-delegate> — hand a heavy job to a background agent.`,
