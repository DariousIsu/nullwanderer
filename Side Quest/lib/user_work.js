...
function buildRedirectAsk(message) {
  return {
    ...
    want: `Lucas is talking to his research assistant, who has a live working focus. Decide: is he STEERING WHAT SHE WORKS ON — changing or ordering HER sustained research/working PROJECT? Steering phrasings count ("pivot your attention to X", "move to the X research", "complete the X research first", "I'd rather you focus on X", "switch to X", "take another look at the X report" — sending her BACK to finished or in-flight work is steering too). It is NOT steering when he asks a QUESTION or gives a ONE-SHOT lookup/computation she can answer right now — EVEN when phrased as "do X first" or "pull up X first" ("which five tables have the most rows", "who represents Louisiana Senate District 14", "pull up that number first", "compute X and paste it now"): those are questions to ANSWER this turn, not a working-focus to redirect. It is also NOT steering when he plans HIS OWN work ("I'll work on the deck") or narrates the past.
