  ...
  try {
    return await ask({
      ...
      want: 'You are Zoe\'s decomposition front door. Parse ONE user utterance into a structured plan. Sort every meaningful token into EXACTLY ONE bucket:\n'
        + '1) OBJECTS — things to look up or act on (a person, org, place, bill, committee, government_body, event, document, claim, concept). op="resolve" if we likely already hold it, op="create" if it is new. salient=true for the ones central to the request. Use the MOST COMPLETE identifier for the mention.\n'
        + '   ⭐ REFERENCES. When a mention POINTS AT something already under discussion — a pronoun (his/her/their/them/it/they) or a definite back-reference ("that list", "the report we made", "the one we discussed") — set `ref`:true and the `type` you expect (person for his/her/them; document for that-report/that-list; else the fitting type), and put the reference WORD itself as the mention ("his", "that list"). Do NOT invent a proper name for a ref — the system binds it to the exact thing already on the table. Name a referent directly (ref false/omitted) ONLY when its real name appears in the utterance itself.\n'
        ...
        + 'Output ONLY JSON: {"intent":"...","objects":[{"mention":"...","type":"person|organization|place|bill|committee|government_body|event|document|claim|concept","op":"resolve|create","salient":true|false,"ref":true|false}],"relations":[{"source":"mention","type":"predicate","target":"mention"}],"constraints":[{"kind":"temporal|speaker|location|other","value":"...","binds":"mention or null"}],"deliverable":"phrase or null","clarify":["..."]}',
      validate: (raw) => {
        const m = String(raw || '').match(/\{[\s\S]*\}/);
        if (!m) return { valid: false, error: 'no json' };
...
function routeDecomposition(parsed) {
  ...
  const objects = (Array.isArray(parsed.objects) ? parsed.objects : []).map(o => {
    ...
    return { mention, type, op: (o && o.op === 'create') ? 'create' : 'resolve', salient: (o && o.salient === true), ref: (o && o.ref === true) };
  }).filter(Boolean);
