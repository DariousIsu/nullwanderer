  ...
  try {
    return await ask({
      ...
      want: 'You are the intake gate for Zoe. Decide whether the user is ASSIGNING A SUSTAINED TASK/PROJECT — work Zoe should carry out over time (research, gather, find, compile, generate, build, monitor, produce a deliverable) — as opposed to asking a question, a status check, or chatting. '
        ...
        + 'CRITICAL: do NOT default to "entity" — a request to understand a TOPIC or predict an OUTCOME is topical/forecast, NOT a contact-gathering run. Only choose "entity" when the deliverable really is a set of named people/orgs (with their details/contacts). '
...
function route(decision) {
  ...
  const kind = ['entity', 'topical', 'forecast'].includes(decision.kind) ? decision.kind : 'entity';
