  ...
  grab('forecast', () => {
    ...
    try {
      ...
      if (now - lastAt >= SCENARIO_FLOOR_H * 3600e3) {
        ...
        if (cat.length) scnOffer = `\n   OR run a hypothetical what-if (move "scenario") — ILLUSTRATIVE only, isolated from this baseline, never memorialized. target a catalog id (${cat.map((s) => s.id).join(', ')}) OR describe your OWN one-line what-if and it is estimated into effects.`;
      }
...
function depthGuidance(tier) {
  ...
  return 'DEEP tick — you have EARNED budget for the highest-value ANTICIPATORY work. IN ORDER, CONDITIONAL ON THE STATE: (1) ONLY IF the state\'s HIS CALENDAR section actually lists a SOON meeting with no prep → BUILD the prep brief. If the state shows NO calendar section or no upcoming meeting, calendar work does not exist this tick — do NOT invent a meeting to prep, and do NOT re-target "upcoming meeting" when your recent ticks show it repeatedly NOT met (there is no meeting to find). (2) HAVE-vs-NEED across your active threads — what MATERIALS/DATA do they REQUIRE that you do NOT yet hold? Go get the most important one (fill-gap/research/build). (3) an explore walk — test whether your own graph and your held documents AGREE about something load-bearing in the state (a single-source cluster, an entity your active work leans on). The best assistant anticipates and comes PREPARED — but only from what is actually there.';
}
