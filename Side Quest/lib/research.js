...
function buildDeepLanePrompt({ goal = '', org = '', facet = '', guidance = '', known = '' } = {}) {
  const g = guidance ? `\n${guidance}\n` : '';
  const body = `You are the DEEP / STRUCTURED-DATA lane for a known organization. Do NOT browse the open web (another lane does that). Use the STRUCTURED tools — IRS 990 financials, federal funding, FEC, and OUR knowledge graph — to surface authoritative records.\n\nORGANIZATION: ${org}\nFACET OF INTEREST: ${facet}\n${g}\nGATHER, where available: (1) nonprofit_lookup → the org's 990 leadership/board + exec-comp + revenue (often names the very officers the facet asks about); (2) gov_funding → federal grants/contracts it receives; (3) fec_lookup → any affiliated committees/PACs; (4) kg_search (+ kg_neighborhood on the id) → what OUR graph already knows about it and its people; (5) knowledge_search → our vault; (6) when the facet asks for a NUMBER or PROBABILITY — a flow total, an overlap count, a likelihood — COMPUTE it with analyze_data (python over what we have banked; import zoe_data; probabilities coded with base rate + assumptions so the estimate is auditable); forecast_query answers ELECTORAL odds only; never estimate a figure in prose, and state exactly what data is missing when it cannot be computed. Ground EVERY detail in what the tools return — never invent; write "not found" for anything missing. Report the structured findings for ${org}.`;
  return require('./known').withKnown(body, { knownBlock: known, entity: org, facet });
}
...
function buildOrganizeEnrichPrompt({ org = '', facet = '', raw = '' } = {}) {
  const label = facetLabel(facet);
  return [
    { role: 'system', content: `You organize raw research findings about ONE named facet for ONE organization into a single clean section. Ground ONLY in the notes — never add a name, title, email, or number not present; write "not found" for anything missing. NEVER use initials, abbreviations, or a placeholder in place of a real name (e.g. "R. Z." or "VP" is NOT a name — write "not found"). Drop any leaked JSON or tool/control text. Dedupe repeats. PRESERVE inline sources: a fact that carries "(source: <url>)" or a named source in the findings keeps that annotation NEXT TO the fact in your output — never strip it and NEVER mint a URL not present in the findings. Output EXACTLY this Markdown and nothing else:\n## ${org || '<organization>'}\n- **${label}:** <named individuals with their roles and direct contact details, one per line; or "not found">` },
...
function buildUnderstandTargetPrompt({ goal = '', target = '', raw = '', known = '', priorDoc = null, sources = [] } = {}) {
  ...
  // ⭐ STAKE TRAVELS WITH THE SOURCE (S1). A URL alone does not tell the model whose interest a page
  // serves, so a company's own figure about itself read exactly like an independent one. Annotate the
  // list the model must cite from, so an interested source is visibly interested AT THE MOMENT OF
  // WRITING rather than being caught later by an editor. Silent on `unknown` — a label with nothing
  // behind it is noise, and the honest default here is to say nothing.
  const _stakeNote = (u) => {
    try {
      const stakeLib = require('./stake');
      const { stake } = stakeLib.classifyStake({ url: u, subject: target });
      if (stake === stakeLib.STAKE.SUBJECT_REPORTED) return `  <- belongs to ${target}: attribute any figure from it to them, never as independent fact`;
      if (stake === stakeLib.STAKE.INTERESTED_ACCEPTED) return '  <- interested party, accepted but not independently audited: attribute it';
      if (stake === stakeLib.STAKE.INDEPENDENT) return '  <- independent record';
      return '';
    } catch { return ''; }
  };
  return [
    { role: 'system', content: `You are the research brain synthesizing ONE target into UNDERSTANDING for a working paper. The gathered notes are evidence, not the deliverable — your job is what they MEAN for the goal.\nOutput EXACTLY this Markdown and nothing else:\n## ${target || '<target>'}\n**What it is & how it works:** <the mechanism in your own words, grounded in the notes>\n**Why it matters to the goal:** <the causal link — what depends on what, and which way it cuts>\n**Key facts & numbers:** <the load-bearing specifics from the notes; never invent a name or number — "not found" for anything missing>\n**Tensions & unknowns:** <where sources disagree; what the notes cannot yet answer>\nThen up to 3 lines, each starting exactly "OPEN: " — what you now realize you DON'T KNOW.\nAn open question names a gap in UNDERSTANDING, not an errand: ask HOW something works, WHY it is that way, HOW MUCH/HOW MANY, WHAT DEPENDS ON IT, WHAT CHANGED and what would happen if it changed, or WHICH of two readings the evidence supports. A question whose answer is a page, an address, a list, or a document location is a FETCH, not an open question — never write one ("where is X published", "what is X's contact page", "which page lists Y"). If the notes left no genuine gap in understanding, write no OPEN line at all.\n⭐ NAME THE SUBJECT IN EVERY OPEN LINE — write "${target || 'the organization'}" in full, NEVER "the lab", "the institute", "the center", "it", "this organization" or "they". These questions OUTLIVE this target: they go into the run's shared plan and are pursued later, when a different organization is being researched. A question that says "the lab" silently rebinds to whichever one is current then, and its answer is filed against the wrong body. Measured live 2026-07-31: "the total dollar amounts … supporting THE LAB", written about the Eller AI Lab, was re-asked against three unrelated institutes in a row. Write the name even when it feels redundant — the redundancy is the point.\n⭐ WHOSE NUMBER IS IT. Some entries in the SOURCES list are annotated with an arrow saying whose interest they serve. A figure that comes from the subject's OWN source is usable but is NOT independent fact: write it as "X reports …" or "according to X's own figures", never as a bare number. The same goes for a figure produced by an interested party and merely accepted by an oversight body — say that it is unaudited. An unannotated source is simply unclassified; do not upgrade it to independent on your own.
Discipline: claims from the notes stay faithful to the notes; your own inference must read as inference ("this implies…", "likely because…"), never as sourced fact. Every load-bearing fact/number carries its source: append "(source: <url>)" chosen ONLY from the SOURCES list, or "(source: gathered notes)" when no listed page carries it — NEVER invent a URL. The notes contain "[pages read this pass: …]" markers — material BELOW a marker came from those pages; use them to bind claims to their URLs. A "[held knowledge this pass: doc:N …]" marker means the material below it came from our OWN verified store — bind such claims "(source: held doc:N)" using refs from that marker ONLY, never an invented ref. Drop any leaked JSON or tool/control text.${priorDoc ? `\nA LIVING DOCUMENT on this topic already exists — DEEPEN, REVISE, or CONTRADICT what it concluded; never restate it. Where the new evidence changes a prior conclusion, say so explicitly ("revises the earlier finding that …").` : ''}` },
    { role: 'user', content: `THE GOAL: ${goal}\n\n${priorDoc ? `WHAT THE LIVING DOCUMENT ALREADY CONCLUDED ("${String(priorDoc.title || '').slice(0, 120)}" — bounce off it, don't restate it):\n"""\n${String(priorDoc.extract || '').slice(0, 3000)}\n"""\n\n` : ''}${known ? `ALREADY IN OUR GRAPH:\n${String(known).slice(0, 2000)}\n\n` : ''}${srcList.length ? `SOURCES (the pages this run actually visited — cite from these):\n${srcList.map((u) => `- ${String(u).slice(0, 160)}${_stakeNote(u)}`).join('\n')}\n\n` : ''}GATHERED NOTES ON ${target}:\n"""\n${String(raw).slice(0, 16000)}\n"""\n\nSynthesize the understanding section now.` }
  ];
}
