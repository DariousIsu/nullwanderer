...
- obs_query {"lane":"heartbeat","since_min":240,"grep":"suppressed"}   the OMNIBUS event bus (obs_events): EVERY organ's lane events — research cites, harvest, transport, anomalies — newest first. The WATCH ORGAN's own findings are ONLY lane="watch" rows (+ the capability_needs ledger); a "what did self-watch flag" question filters lane:"watch" — research-lane [cite] rows are citation exhaust, NEVER self-watch findings
...
const READ_SAFE = new Set([
  ...
  'kg_search', 'kg_neighborhood', 'knowledge_search', 'gov_funding', 'fec_lookup', 'fec_candidate',
