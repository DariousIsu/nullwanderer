...
- analyze_data {"code":"import zoe_data\nprint(zoe_data.tables('sq'))\ncols, rows = zoe_data.query('sq', 'SELECT COUNT(*) FROM documents')\nprint(rows)"}   RUN a one-off python ANALYSIS over her OWN data, READ-ONLY (the Echo venv's python + pandas). \`import zoe_data\`; \`zoe_data.dbs()\` lists the stores (sq = short-term memory, graph = the civic KG, news, puller, electoral = the CRM); \`zoe_data.tables(db)\` / \`zoe_data.schema(db, table)\` DISCOVER the shape — always look before you query, never guess a table name; \`zoe_data.query(db, sql, params)\` → (cols, rows). Writes are REJECTED; the run is ephemeral and your prints ARE the result. Reach for this when a plain lookup can't answer — aggregate / cross-tabulate / compute over her data
...
- rehearsal_drive_start {"slug":"my-idea","goal":"<one sentence: the change to attempt>","suite":"smoke_x.js","files":["lib/x.js"]}   journal an ITERATING rehearsal run (one at a time) — the loop then edits, tests, and reads its own failures across ticks. rehearsal_drive_iterate {} advances it one bounded step; rehearsal_drive_status {} reads where it stands. Green ends as a PROPOSAL CARD document — nothing ever self-adopts`;
