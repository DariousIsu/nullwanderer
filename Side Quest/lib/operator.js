...
- analyze_data {"code":"import zoe_data\ncols, rows = zoe_data.query('sq', 'SELECT count(*) FROM messages')\nprint(rows)"}   RUN a one-off python ANALYSIS over her OWN data, READ-ONLY (the Echo venv's python + pandas). \`import zoe_data\`; \`zoe_data.query(db, sql, params)\` → (cols, rows) over the whitelisted DBs (\`zoe_data.dbs()\` lists them; 'sq' = her short-term memory). Writes are REJECTED; the run is ephemeral and your prints ARE the result (nothing is adopted). Reach for this when a plain lookup can't answer — aggregate / cross-tabulate / compute over her data
