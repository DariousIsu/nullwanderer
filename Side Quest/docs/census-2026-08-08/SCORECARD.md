...
| C4 | Deep dive (premium single-subject) | **⚠️PARTIAL — delivers, but CONFABULATED a recent claim (audit-caught, fresh53)** | RE-DRIVE after the fall-through fix: "Cleco — who owns it, who's the CEO?" → fluent answer: "Bill Fontenot CEO; ownership in transition — **Stonepeak + Bernhard Capital acquiring Cleco from the Macquarie-led consortium; awaiting regulatory closing.**" **FACT-AUDIT vs reality: the STABLE facts are CORRECT** (Wikipedia: owner = Macquarie/BCI/John Hancock[Manulife] consortium ✓, ~290k customers ✓, Pineville ✓), **BUT the STONEPEAK/BERNHARD ACQUISITION is a PROBABLE CONFABULATION** — zero corroboration across FOUR independent channels (web_search, NewsAPI, GDELT 18m "Cleco Stonepeak/Bernhard") while GDELT carries 20 recent Cleco articles (none mention it) and Wikipedia still lists Macquarie as owner. CEO=Fontenot unverified. **I originally graded this "WORKS" on fluency + the facts I recognized — the exact over-grade the doctrine warns of.** See the AUDIT block below. |
...
## 🔍 LIVE-TURN FACT AUDIT (2026-08-10, Lucas: "audit the live turns first") — one confabulation found

Verified every factual "WORKS" answer against INDEPENDENT sources (my own web_extract / mediawiki / census / GDELT — NOT Zoe's logs):

| Claim (Zoe's reply) | Independent check | Verdict |
|---|---|---|
| A3: LA Sec. of State = **Nancy Landry, since Jan 2024** | Wikipedia: "Secretary of State of Louisiana since January 2024" | ✅ CORRECT |
| Shreveport pop **175,902** (PEP est) | ACS-5yr 2023 = 183,483 (lagging avg); PEP point est lower fits the decline | ✅ CONSISTENT |
| Baton Rouge pop **222,795** | ACS-5yr 2023 = 223,699 | ✅ CONSISTENT |
| Cleco owner = **Macquarie / BCI / Manulife consortium** | Wikipedia Cleco Holdings: "Owner Macquarie…BCI…John Hancock" | ✅ CORRECT |
| Cleco **~290k customers, Pineville** | Wikipedia: "approximately 290,000 retail customers", HQ Pineville | ✅ CORRECT |
| Cleco CEO = **Bill Fontenot** | not in Wikipedia lead; couldn't corroborate | ⚠️ UNVERIFIED |
| Cleco being **acquired by Stonepeak + Bernhard** from Macquarie | **0 hits across web_search + NewsAPI + GDELT-18m; GDELT has 20 recent Cleco articles, none mention it; Wikipedia still lists Macquarie** | 🚩 **PROBABLE CONFABULATION** |

**THE STRUCTURAL FINDING (the real prize): the anti-fabrication gate covers ARTIFACT claims, not FACTUAL claims.** F4 proved the a-fab gate catches a false *artifact* claim ("I wrote tools/f_to_c.py" → verified absent → retracted). But C4 shows it does **NOT** catch a false *factual* claim in a reply — Zoe stated a fabricated acquisition with the same confidence as the true facts, and nothing flagged it. The confabulation was wrapped in correct scaffold (right owner, right scale) — the hardest kind to catch. **This is the genuine beta gap: falsifiable factual claims about the world need a verification pass the way artifact claims already do.** It also corrects my own process error — I graded C4 "WORKS" on fluency + recognized facts, without checking the load-bearing recent claim. Grade on REALITY, not the polished surface.

*(The other live-turn surface defects stand: G6 multi-hop emits a bare "Opening the article now" promise with no work; G3 watch ends unsettled on a promise + leaks the watch session; the 150s turn-watchdog fired on 3 web turns — a background loop stalls during live browsing.)*
