...
*19:46 — the director's look words (cut 13's gaze half), folded in — the last item on his list.* She looks at him when she speaks to him or listens, and away when she thinks: a look vocabulary beside the posture clips, a pure gaze target (the camera's face when fresh, else ahead; away = aside and up), the chat page's own two events carried through main to every window, the companion's look-at bone honouring the look for a moment, the 2D face's pupils following. smoke_avatar_look 16. With this, every cut of the wants plan stands at v1 except 9C (RAM) and his levers.

*19:38 — cut 12, the workshop, and cut 1's pen seam, folded in.* The workshop: a weekly frontier-class slot (one piece a week, his lever) on a deep idle tick — a draft, a critic who reads for pulse, specificity and the ending with no citation asked, three passes at most; the piece lands in documents (ref workshop, never the road), in a notes file he can find unannounced, in her own changes and her narrative; his "I kept reading" or "I stopped" is the judge, in the landed ledger; a Kokoro reading on request. The pen seam: a proposal touching a persona file mints her card at propose time, holds for her yes beside his ✓, her no retires it, the landing advances the manifest. smoke_workshop 29 · smoke_code_pen +9.

*19:27 — cut 11, the organ atlas, folded in.* Measured: 500 organs, 132 with a log prefix, 53 with a switch, 416 with a smoke. lib/organ_atlas.js is a read model over the self audit's corpus: a seeded table of every organ with its file, lane, prefixes, switches, meta keys, smoke and header, refreshed by the audit's sweep (a vanished file is a stale entry and a finding, never a delete). "Which organ owns my mood" resolves to lib/mood.js with its switch and smoke; the source map leads with the organ a question names; the self-question brief carries the atlas line; the hard test has a which-organ kind. smoke_organ_atlas 21.
...
- **09-06 ~09:00 — THE CAMERA WAS BLIND FOR 18 HOURS AND NOTHING SAID SO (his words: "The camera is still not seeing when I sit back down at the computer or she's not processing that I am here"):** the machine's hard reset at 15:13 on 09-05 left the C920 un-enumerated on USB (Windows: all three device nodes code 45, not connected; no physical camera present; the LED lit by port power), and a default getUserMedia picked a VIRTUAL source ("AMD Privacy View camera" / "OBS Virtual Camera") that streams black — 1,174 readings on p328 alone said `present=0 faces=0`, her manifest said "no one is in front of the camera", and the reach went to Discord while he sat at the desk. CURED: renderer/chat.js picks a PHYSICAL camera by label (a remembered `camera.device_label` first; VIRTUAL_RE never watched), says an absent camera once per 10 min and looks again on the device-change event and every 30 s, and tells main the device; main logs `camera ON — through <label>` / `camera ABSENT — <why>` and hands the device to the sense; sidecar/face_embed.py rides the frame's mean luminance on every result; lib/face_sense reads a mean under DARK_MEAN (8) as `dark` (log `CAMERA DARK` once per 10 min; the line says "dark — the lens shows only black", or "none is connected" when the renderer said so); lib/presence_state treats a dark frame as no reading (never an empty chair); lib/consciousness takes no face percept from a dark frame. Pins: smoke_face_sense +11 (the dark reading and line, the absent device, presence under a dark frame, the wiring). THE CAMERA ITSELF is his to reconnect (replug the C920).
