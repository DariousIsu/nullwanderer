...
4. **Frontier judgment at every hop.** The agent loop is driven by the cloud model — query reformulation, source triage, failure diagnosis, slot decomposition. Recipe ladders and local models are sub-tools it may call, never the driver. (Local = absolute extreme last resort, per the 08-21 rule.)
