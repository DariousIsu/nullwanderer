...
- **Goldilocks, not monoculture.** A failed cultivation still advances the cursor (no permanent-DUE lock); pressure resets on each attend so it recurs without dominating. **The floor is enforced in the executor, not merely advised** (see the live finding below).

## Live finding + fix (boot_c4, 2026-08-11) — the DUE gate
First live boot proved the move fires (`chose=attend-self → mood=1 thought=1`, on a tick where research + the directed pass were both quota-deferred; grounded non-template mood + a real inner thought #88428; firewall held). But it **over-fired** — 4× in ~50 min, not once per ~6h. Two coupled causes: (1) the 6h floor was only *advisory* in `DECISION_WANT` (`validateDecision` can't see the live cursor), so when the task lanes were quota-throttled the decider kept picking `attend-self` as the available non-deferred move; (2) `personaAttend` calls `mood.compose` directly (bypassing the 90-min TTL), re-cultivating on every pick. Net: persona flipped into its own mini-monoculture — the inverse of the drift it cures — with redundant cloud spend.
**Fix:** the `main.js` executor now hard-gates on `autonomy.personaPressure({ lastAttendAt, now }).due` (mirroring `engage`'s cadence gate). Not due → skip cheaply (no cultivation), record a "skipped — not yet due" history line (which discourages the decider from re-picking). The 6h floor is now real regardless of what the cloud chooses, and the redundant `mood.compose` churn is gone. `smoke_c4_persona` gained the DUE-gate contract assertions (27/27).
