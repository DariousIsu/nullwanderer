...
| **B1 — weights land** | Download the lexicons (⚠ needs Lucas's download OK), load as sq.db tables + a lookup lib | licenses verified §3d; gate = row counts + spot-check known words |
| **B2 — appraisal tissue** | Clean-room python per L2, oracle-tested against the scratchpad references | gate = fixed-input trajectory reproduction + R3-posture proof (write attempt SQLite-rejected) |
| **B3 — impression tissue** | §4 schema + landing door + manifest injection | gate = same-subject-same-history determinism (the hard-test law) + reasons[] non-empty on every row |
| **B4 — mood renders the vector** | the 08-14 proposal's Slice 1, unchanged | side-by-side week; identity firewall smoke unchanged |
