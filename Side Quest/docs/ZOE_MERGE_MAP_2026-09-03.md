...
| 18 | Trajectory logging | cloud_traces, agent_events, obs_events | agent_trajectory with OpenInference columns | One trajectory contract: a per-task record joining components to outcome; the mining organ's input (the run ledger is that record; lib/trajectory_mine reads it for recurring failure classes, brief at GET /trajectory) | Leg B | Partial — organ landed 09-04 |
