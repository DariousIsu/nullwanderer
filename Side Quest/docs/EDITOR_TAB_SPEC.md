...
6. ✅ DONE (`d523221` wiring, `2335a18` live-fire) — harness drives "Run checks". `studio/verify_harness.js` = the one-pathway orchestrator (smoke 14/14); `studio/verify_model_io.js` = the model-leaf adapters (homework parse + classify STATUS=/NOTE= parse; smoke 8/8); `lib/ollama.complete()` non-streaming; `lib/editor_checks.runHarnessChecks` builds the real injections (callTool=echo client, embed/cosine=lib/memory, classify+homework=local 24B via complete, frontier optional) + records the check_run (agent `runChecks` kept as fallback). `main.js editor:run-checks` swapped to it. **LIVE-FIRE PROVEN** (`scripts/livefire_harness.js`, real engine + real local 24B + real bge-small, 3.7s): URL claim → fetch (web_extract, 2000 chars) → match gray 0.736 (Tier B embeddings) → preflight homework-check (local 24B, passed) → classify V (local 24B) → contract, 0 invalid; model reached twice, LOCAL-ONLY, zero cloud tokens.
   - **FETCH-RUNG TOOL = `web_extract`** (trafilatura clean text + status), NOT web_fetch (raw-HTML preview); both expose body under `text_preview` (readFetch reads it).
   - ⚠️ **ENGINE GAP (not the harness): `web_search` has no provider key** (exa/brave/duckduckgo `no_key_or_error`) → claims WITHOUT a URL can't resolve via the search rung. Add a search-provider key to Echo to light up search-based resolution. The cloud frontier classify tier is wired but OFF by default (local-first); enable by passing frontierModel/base+key.
   - REMAINING: restart Zoe so the IN-APP button loads the wired handler (live-fire proved the code path already); then B4/B5 (cert issuance + template) consume `{findings, suggestions, summary}`.
