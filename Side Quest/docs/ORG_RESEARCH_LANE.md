...
- `runOrgMove(deps)` — one org per move: pick → resolve URL → `fetchPage(url)` (injected) → `org_site.verifyPage` → `land({name,url,text})` (injected → `doc_store.land({source:'org_research',origin:url})` since `7990c4b` — importance stamped (6), C2 pressure, content-dedup; the existing decompose lane extracts entities/relations, **no second extraction stack**) → `markResearched` (injected → an `official_site` belief = the durable done-marker) → attempt cooldown. Every I/O injected → fully offline-testable.
