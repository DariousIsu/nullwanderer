...
- ✅ **C4 — persona-anchored drive: BUILT** (this commit; spec `SPINE4_C4_PERSONA_DRIVE.md`). Existing-organ finding — the idle decider (`lib/autonomy.js`) chose research ~100% because its manifest was ~20 task sections and ZERO self; the defensive drift fixes (self_model guards, mood layer) were all in place but persona had no seat at the *decision*. Cure (positive, PEPA): persona is now a first-class COMPETING DRIVE. `personaPressure` (pure) rises with time since she last tended to herself → DUE past a Goldilocks floor (6h, `ZOE_PERSONA_FLOOR_H`), capped 3× (deep neglect out-competes a busy research state). A `persona` manifest section (first) surfaces who-she-is + how-she-feels + a DUE flag; new move `attend-self` (target/expect-free, why-required); `personaAttend` (pure, dep-injected) cultivates the MOOD layer (`mood.compose` — grounded, invents no events) + lands a private inner thought + advances the cursor. **Honesty invariant holds BY CONSTRUCTION:** attend-self never imports `self_model` (identity firewall), never speaks (only `engage` does → cannot override Spine 2), invents nothing (mood.js owns that), and a failed cultivation still resets the cadence (no permanent-DUE monoculture). `smoke_c4_persona` **25/25** (incl. a self_model-write spy asserting 0 writes). *Live bar (watch-and-wait): `[autonomy] chose=attend-self` recurs over a multi-day idle window; inner life no longer ~100% research; zero Spine-2 regression.*
