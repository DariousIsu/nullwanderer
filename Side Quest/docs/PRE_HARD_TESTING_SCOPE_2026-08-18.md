...
### Wave 2 — Reproducibility: the governed `entropy` module  ✅ SHIPPED + LIVE-VERIFIED 2026-08-18
...
- **LIVE DRILL (real `gemma4:12b` on the 7900 XT, headless):** deterministic mode → **byte-identical** across two runs (governed opts `{"temperature":0,"seed":…}`, 134 chars exact); prod → two different sentences. **The hardware decodes deterministically at temp 0 — byte-identity is achievable, not defeated by GPU nondeterminism.** The prod boot logs its crypto seed once (`[entropy] mode=prod seed=0x… — replay`), so any live session is replayable post-hoc.
...
2. Wave 2 live: **DONE at the model level (2026-08-18).** The real local model is byte-identical across two deterministic-mode runs of the same prompt (headless drill vs `gemma4:12b`). ⚠️ **Methodology finding:** the full `:8767` turn diff is confounded by PERSISTENT conversation state — two reboots don't share a fresh context (boot A's turn lands in `sq.db` before boot B fires, and the 120s active-window also blocks a rapid second turn), so a clean full-turn byte-diff needs a conversation-state-reset harness, not a reboot. The model-level determinism that hard testing actually rests on is proven; the full-turn harness is a separate (small) build if we want end-to-end turn diffing.
